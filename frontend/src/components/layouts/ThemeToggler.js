 ...
 * It uses local storage to remember the user's theme preference and updates
 * the document's class list accordingly.
 */

import { useEffect, useRef } from "react";

export default function ThemeToggler() {
  // Refs to access the toggle button and theme icons
  const themeToggleBtn = useRef();
  const themeToggleLightIcon = useRef();
  const themeToggleDarkIcon = useRef();

  // Effect to set the initial theme based on local storage or system preference
  useEffect(() => {
    if (
